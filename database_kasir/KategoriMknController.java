package database_kasir;

import javafx.event.ActionEvent;
import javafx.fxml.FXML;
import javafx.scene.control.Button;
import javafx.scene.input.MouseEvent;
import javafx.scene.layout.Pane;
import javafx.scene.text.Text;

public class KategoriMknController {

    @FXML
    private Button btnBarang;

    @FXML
    private Button btnHapusKtgMkn;

    @FXML
    private Button btnMakanan;

    @FXML
    private Button btnTambahKtgMkn;

    @FXML
    private Button btnUbahKtgMkn;

    @FXML
    private Pane paneMenu;

    @FXML
    private Text txtHome;

    @FXML
    private Text txtMakanan;

    @FXML
    void barangMenu(ActionEvent event) {

    }

    @FXML
    void hapusKtgMknMenu(ActionEvent event) {

    }

    @FXML
    void homeMenu(MouseEvent event) {

    }

    @FXML
    void makananMenu(MouseEvent event) {

    }

    @FXML
    void tambahKtgMknMenu(ActionEvent event) {

    }

    @FXML
    void ubahKtgMknMenu(ActionEvent event) {

    }

}
