package database_kasir;

import javafx.event.ActionEvent;
import javafx.fxml.FXML;
import javafx.scene.control.Button;
import javafx.scene.input.MouseEvent;
import javafx.scene.layout.Pane;
import javafx.scene.text.Text;

public class PenjualanMknController {

    @FXML
    private Button btnBarang;

    @FXML
    private Button btnHapusPjlMkn;

    @FXML
    private Button btnMakanan;

    @FXML
    private Button btnTambahPjlMkn;

    @FXML
    private Button btnUbahPjlMkn;

    @FXML
    private Pane paneMenu;

    @FXML
    private Text txtHome;

    @FXML
    private Text txtMakanan;

    @FXML
    void BarangMenu(ActionEvent event) {

    }

    @FXML
    void hapusPjlMknMenu(ActionEvent event) {

    }

    @FXML
    void homeMenu(MouseEvent event) {

    }

    @FXML
    void makananMenu(MouseEvent event) {

    }

    @FXML
    void tambahPjlMknMenu(ActionEvent event) {

    }

    @FXML
    void ubahPjlMknMenu(ActionEvent event) {

    }

}
